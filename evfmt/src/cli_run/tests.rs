...
fn policy_key_set_lists_parse_shortcuts_and_comma_lists() {
    let all = parse_policy_key_set_list(UpdateKind::Add, "all").unwrap();
    assert!(all.contains('#'));
    assert!(all.contains_keycap('#'));
    assert!(all.contains('\u{00A9}'));

    assert_eq!(
        parse_policy_key_set_list(UpdateKind::Set, "none").unwrap(),
        PolicyKeySet::none()
    );
    ...
    let text_defaults = parse_policy_key_set_list(UpdateKind::Add, "text-defaults").unwrap();
    assert!(text_defaults.contains('\u{00A9}'));
    assert!(!text_defaults.contains('\u{2728}'));

    let emoji_defaults = parse_policy_key_set_list(UpdateKind::Add, "emoji-defaults").unwrap();
...
#[test]
fn policy_key_set_lists_reject_invalid_shortcut_usage() {
    assert_parse_error(
        parse_policy_key_set_list(UpdateKind::Add, "none"),
        "`none` is only allowed",
    );
    assert_parse_error(
        parse_policy_key_set_list(UpdateKind::Set, "all,ascii"),
        "`all` and `none` must appear alone",
    );
    assert_parse_error(parse_policy_key_set_list(UpdateKind::Set, ""), "empty list");
    assert_parse_error(
        parse_policy_key_set_list(UpdateKind::Set, "ascii,"),
        "empty list item",
    );
}
...
fn policy_key_set_items_report_specific_errors() {
    ...
    assert_parse_error(
        parse_policy_key_set_list(UpdateKind::Set, "u(00ag)"),
        "invalid code point item",
    );
    assert_parse_error(
        parse_policy_key_set_list(UpdateKind::Set, "u(00A9"),
        "invalid code point item",
    );
    assert_parse_error(
