...
A scanner that uses valid emoji skeletons rather than exact RGI membership can recognize useful local structure even when the `FE0E`/`FE0F` spelling is different, redundant, conflicting, missing, or misplaced. That gives later analysis the context it needs to decide whether each selector is owned, redundant, conflicting, defective, forced, or unsanctioned.
...
The weak point is whether this structural permissiveness earns its complexity. It makes scanner behavior harder to audit, can group ordinary-looking text as emoji-related structure, and can create unbounded scan items. The buffering risk is not limited to ZWJ-related sequences: ZWJ-related items, tag runs, and other repeated selector-bearing structures can all grow without a fixed bound. Forward-only scanning over a complete input prevents backtracking and repeated work, but it does not by itself guarantee constant-size buffering or true input streaming. The argument would weaken if a simpler scanner could expose every selector and preserve the same selector-only idempotence without representing these valid emoji skeletons.
