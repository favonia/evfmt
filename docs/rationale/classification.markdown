...
Historical boundary: the published UTR #51 Revisions 3 and 5, along with the surrounding draft Revisions 2, 4, and 6, defined ED-13 as `(emoji_modifier_base | emoji_base_variation_sequence) emoji_modifier`. The `emoji_base_variation_sequence` branch required the base and `FE0F` to form a valid variation sequence; the accompanying guidance classified support for any other base-selector pair as non-conformant. Revision 7, published with Unicode Emoji 3.0, narrowed ED-13 to `emoji_modifier_base emoji_modifier` and introduced the compatibility note about older data containing an intervening selector. The historical record most strongly supports reading “older data” as sanctioned `Emoji_Modifier_Base FE0F Emoji_Modifier` spellings. A broader reading of the current sentence could include unsanctioned `FE0F` forms that the earlier grammar did not accept.

The sanctioned-selector and modifier-base boundaries produce four cases:

| `FE0F` sanctioned for the base | Base has `Emoji_Modifier_Base` | Earlier UTS #51 ED-13                                                       | Current UTS #51 description                                                                                                   | `evfmt` accounting               |
| ------------------------------ | ------------------------------ | --------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- | -------------------------------- |
| yes                            | yes                            | Accepted through the `emoji_base_variation_sequence` branch                 | UTS #51 describes this as defective older data and directs implementations to ignore `FE0F`                                   | `modifier_defective_selectors`   |
| yes                            | no                             | Not an emoji modifier sequence                                              | `base FE0F` is an emoji presentation sequence; the following modifier does not form an emoji modifier sequence with this base | `additional_defective_selectors` |
| no                             | yes                            | Not accepted because `base FE0F` was not an `emoji_base_variation_sequence` | A broad reading of current UTS #51 could include this row. The older-data history supports only the sanctioned row above      | `unsanctioned_selectors`         |
| no                             | no                             | Not an emoji modifier sequence                                              | `base FE0F` is not an emoji presentation sequence, and the modifier does not form an emoji modifier sequence with this base   | `unsanctioned_selectors`         |
...
Accounting inference: the historical standard defined the legacy modifier spelling through a sanctioned variation sequence and classified other base-selector pairs as non-conformant. The archived examples follow the same boundary. `evfmt` therefore uses `modifier_defective_selectors` when the base has `Emoji_Modifier_Base` and `FE0F` is sanctioned for that base. It uses `unsanctioned_selectors` when `FE0F` is unsanctioned. Both paths remove the selector and preserve the same base-plus-modifier output.
