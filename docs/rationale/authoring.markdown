...
When a rationale depends on another layer, state the dependency as a given condition or as a cross-layer check. If the combined behavior is unacceptable, the possible outcomes include changing one layer, changing several layers, changing all affected layers, changing the public policy surface, narrowing recognition, or documenting a real evidence gap. Do not use one rationale entry to make a circular proof that the current layers are correct.
